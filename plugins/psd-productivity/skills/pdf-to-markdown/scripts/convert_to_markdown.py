...
def setup_api_key():
    """Load Gemini API key and set as env var. Marker expects GOOGLE_API_KEY."""
    if "GOOGLE_API_KEY" in os.environ:
        return

    # Try the shared secrets module
    try:
        sys.path.insert(0, str(Path(__file__).parent.parent.parent.parent / "scripts"))
        from secrets import get_secret
        value = get_secret("GOOGLE_API_KEY") or get_secret("GEMINI_API_KEY")
        if value:
    ...
    print(
        ...
        "Or in ~/.config/psd-productivity/.env:\n"
        "  GOOGLE_API_KEY=your-key-here\n\n"
        "See SECRETS-SETUP.md for full instructions.",
        file=sys.stderr,
    )
    sys.exit(1)
